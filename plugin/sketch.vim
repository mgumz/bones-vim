...
" -----------------------------------------------------------------------
" x bug fixes, plus dragging (v 0.2)
" - more bug fixes
" >>final beta release (v 0.2.4)
...
fun! PasteTransparent() range
   ...
   if b:above[0] == ' '
      let above = substitute(above, ' ', below[0], '')
   endif
   let i = 1
   while i < strlen(above)
      if b:above[i] == ' '
         let above = substitute(above, '\(.\{'.i.'}\).', '\1'.below[i], '')
      endif
      let i = i + 1
   endwhile
   let @p = above
...
" algorithm imperatively, using a couple of explicit stacks. This also seems
" to speed things up.
"
